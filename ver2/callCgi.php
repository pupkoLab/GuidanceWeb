<!--
	callCgi.php : created by Josef Sprinzak
	action of Submit from index.php
	creates run results directory based on time appended to rand 4 digit number_format
	passes POST input parameters as query string to guidance.V2.cgi
-->
...
<?php 
  ...
  if ($size == 0) {
	exit("callCgi.php not called from server home<br>\n");
  }

  $cgi_page = "/cgi-bin/guidance.V2.02.cgi";
  ...
  foreach ($_POST as $key => $value) {
	...
	if (($from_MAFFT == 1) and ($key == "MAFFT_FASTA")) {
		$contents = $value;
		continue;
	}
  ...
  function create_new_directory($dir_path, $dir_name){
	$to_return;
	if(!(mkdir($dir_path.$dir_name, 0777))):
